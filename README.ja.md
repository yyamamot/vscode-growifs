...
[English](https://github.com/yyamamot/vscode-growifs/blob/main/README.md) | 日本語

`vscode-growifs` は、[GROWI](https://growi.org/) のページを VS Code から閲覧、編集、探索するための Desktop 向け VS Code 拡張です。
...
| LLM Assist Kit | ローカルミラー編集と SCM diff review 用の Skills / prompt を生成します |
...
### LLM Assist Kit（実験的機能）

LLM Assist Kit は、ローカルミラーを LLM に渡すための prompt と diff review context を生成する補助機能です。LLM が読んでよいファイル、編集してよい Markdown、実行してはいけない GROWI / SCM 操作を artifact 内で明示します。

基本の流れは次の通りです。

1. `GROWI: Install LLM Local Mirror Skills` で `.agents/skills/growi-local-mirror-prompt/` と `.agents/skills/growi-local-mirror-diff/` を生成する
2. ローカルミラーを同期してから `GROWI: Prepare LLM Local Mirror Prompt` を実行し、`.growi-agent/prompt/current/` を LLM に渡す
3. 差分確認後に Source Control view の `GROWI: Prepare LLM Local Mirror Diff` を実行し、`.growi-agent/diff/current/` を diff review 用に渡す

生成される主なファイルは次の通りです。

| 出力先 | 内容 |
| --- | --- |
| `.agents/skills/growi-local-mirror-prompt/**` | LLM にローカルミラー編集境界を伝える skill |
| `.agents/skills/growi-local-mirror-diff/**` | SCM diff context を読み取るための read-only skill |
| `.growi-agent/prompt/current/task.md` | LLM に渡す作業依頼 |
| `.growi-agent/prompt/current/editable-files.txt` | 編集してよい既存 Markdown の一覧 |
| `.growi-agent/prompt/current/do-not-edit.txt` | 読取・編集禁止対象 |
| `.growi-agent/prompt/current/prompt.md` | LLM に渡す統合 prompt |
| `.growi-agent/diff/current/scm-state.json` | SCM snapshot の構造化 context |
| `.growi-agent/diff/current/changed-files.txt` | 差分対象 Markdown の一覧 |
| `.growi-agent/diff/current/diffs/*.patch` | local / GROWI 側の unified diff |
| `.growi-agent/diff/current/prompt.md` | diff review 用 prompt |

`.growi-agent/` は生成 artifact です。`.gitignore` に `.growi-mirrors/` と `.growi-agent/` を追加する補助確認が出る場合がありますが、`.agents/skills/**` は ignore しません。
...
| `GROWI: Install LLM Local Mirror Skills` | LLM Assist Kit の skill pack を生成する |
| `GROWI: Prepare LLM Local Mirror Prompt` | 編集用 prompt artifact を `.growi-agent/prompt/current/` に生成する |
| `GROWI: Prepare LLM Local Mirror Diff` | SCM snapshot から diff review artifact を `.growi-agent/diff/current/` に生成する |
